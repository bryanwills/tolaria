...
"#;
    ...
    #[test]
    fn test_save_note_content_nested_path_with_spaces() {
        let dir = TempDir::new().unwrap();
        let nested = dir.path().join("Area/Progetti Personali");
        fs::create_dir_all(&nested).unwrap();
        let file_path = nested.join("La mia nota.md");
        let original = "---\ntype: Note\n---\n# La mia nota\n\nContenuto originale.";
        fs::write(&file_path, original).unwrap();

        let edited = "---\ntype: Note\n---\n# La mia nota\n\nContenuto modificato e salvato.";
        let result = save_note_content(file_path.to_str().unwrap(), edited);
        assert!(result.is_ok());

        let saved = fs::read_to_string(&file_path).unwrap();
        assert_eq!(saved, edited);
        assert!(saved.contains("Contenuto modificato e salvato"));
    }

    #[test]
    fn test_save_note_content_roundtrip_read_edit_write() {
        let dir = TempDir::new().unwrap();
        let file_path = dir.path().join("roundtrip.md");
        let original = "---\ntype: Project\nstatus: Active\n---\n# Roundtrip Test\n\nOriginal body here.";
        fs::write(&file_path, original).unwrap();

        // Read → edit → write → read again
        let content = get_note_content(file_path.to_str().unwrap()).unwrap();
        assert_eq!(content, original);

        let edited = content.replace("Original body here.", "Edited body persisted.");
        save_note_content(file_path.to_str().unwrap(), &edited).unwrap();

        let re_read = get_note_content(file_path.to_str().unwrap()).unwrap();
        assert_eq!(re_read, edited);
        assert!(re_read.contains("Edited body persisted."));
    }
